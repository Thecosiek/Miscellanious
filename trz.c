...
int sprawdz_dlugi_przedzial(int * siec, int * odl, int poczatek, int koniec, int maxp) { //sprawdza max przedzialow
  if (siec[poczatek] == siec[koniec]) { // dla danych A, C  i wszystkich możliwych B													
    return maxp;
  }
...
int krotki_przedzial(int * siec, int * odl, int * ostatni, int * nastepny, int n) {
  int krotki = MAXIMUM;
  for (int x = 1; x < n - 1; x++) { //przechodzimy po wszystkich najbliższych polozeniem sieciach
    int A = ostatni[x]; //
    int C = nastepny[x];
...
int main() {
  ...
  if (czy_trzy_sieci(siec, n) == 0) { //sprawdzamy czy sš przynajmniej 3 sieci moteli
    k = 1;
  }
